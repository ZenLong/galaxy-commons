package com.saysth.commons.quartz.mongo;
...
public class MongoDBJobStore implements JobStore {
	...
	public void initialize(ClassLoadHelper loadHelper, SchedulerSignaler signaler) throws SchedulerConfigException {
		...
		options.safe = true; // need to do this to ensure we get DuplicateKey
								// exceptions
	...
	protected OperableTrigger toTrigger(TriggerKey triggerKey, DBObject dbObject) throws JobPersistenceException {
		...
		try {
			Class<OperableTrigger> triggerClass = (Class<OperableTrigger>) loadHelper.getClassLoader().loadClass(
	...
	public List<OperableTrigger> acquireNextTriggers(long noLaterThan, int maxCount, long timeWindow)
		...
		while (cursor.hasNext() && maxCount > triggers.size()) {
			...
			try {
				...
				TriggerKey key = new TriggerKey((String) dbObj.get(TRIGGER_KEY_NAME),
