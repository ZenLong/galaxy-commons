 ...
 * <code>org.quartz.Scheduler</code>). This allows you to create new jobs and
 * triggers, and also to control and monitor the entire Scheduler.
 * 
 * <p>
 ...
 * 
 * <p>
 * <b>Note:</b> This version of Spring's SchedulerFactoryBean requires Quartz
 * 1.5.x or 1.6.x. The "jobSchedulingDataLocation" feature requires Quartz 1.6.1
 * or higher (as of Spring 2.5.5).
 * 
...
public class SchedulerFactoryBean extends SchedulerAccessor implements FactoryBean<Scheduler>, BeanNameAware,
	...
	/**
	 * Return the TaskExecutor for the currently configured Quartz Scheduler, to
	 * be used by LocalTaskExecutorThreadPool.
	 * <p>
	 * This instance will be set before initialization of the corresponding
	 * Scheduler, and reset immediately afterwards. It is thus only available
	 * during configuration.
	...
	/**
	 * Return the DataSource for the currently configured Quartz Scheduler, to
	 * be used by LocalDataSourceJobStore.
	 * <p>
	 * This instance will be set before initialization of the corresponding
	 * Scheduler, and reset immediately afterwards. It is thus only available
	 * during configuration.
	...
	private Map<? extends String, ? extends Object> schedulerContextMap;
	 ...
	 * <code>quartz.properties</code> from <code>quartz.jar</code>. To use
	 * custom Quartz properties, specify the "configLocation" or
	 * "quartzProperties" bean property on this FactoryBean.
	 * 
	 * @see org.quartz.impl.StdSchedulerFactory
	...
	/**
	 * Set the location of the Quartz properties config file, for example as
	 * classpath resource "classpath:quartz.properties".
	 * <p>
	 * Note: Can be omitted when all necessary properties are specified locally
	 * via this bean, or when relying on Quartz' default configuration.
	 * 
	 * @see #setQuartzProperties
	 */
	 ...
	 * <p>
	 * Can be used to override values in a Quartz properties config file, or to
	...
	/**
	 * Set the Spring TaskExecutor to use as Quartz backend. Exposed as thread
	...
	/**
	 * Set the default DataSource to be used by the Scheduler. If set, this will
	 ...
	 * <p>
	 * A Spring-specific subclass of Quartz' JobStoreCMT will be used. It is
	 * therefore strongly recommended to perform all operations on the Scheduler
	 * within Spring-managed (or plain JTA) transactions. Else, database locking
	 * will not properly work and might even break (e.g. if trying to obtain a
	 * lock on Oracle without a transaction).
	 * <p>
	 * Supports both transactional and non-transactional DataSource access. With
	 * a non-XA DataSource and local Spring transactions, a single DataSource
	 * argument is sufficient. In case of an XA DataSource and global JTA
	 * transactions, SchedulerFactoryBean's "nonTransactionalDataSource"
	 * property should be set, passing in a non-XA DataSource that will not
	...
	/**
	 * Set the DataSource to be used by the Scheduler <i>for non-transactional
	 * access</i>.
	 * <p>
	 * This is only necessary if the default DataSource is an XA DataSource that
	 * will always participate in transactions: A non-XA version of that
	 * DataSource should be specified as "nonTransactionalDataSource" in such a
	 * scenario.
	 * <p>
	 * This is not relevant with a local DataSource instance and Spring
	 * transactions. Specifying a single default DataSource as "dataSource" is
	 * sufficient there.
	 * 
	 * @see #setDataSource
	 * @see LocalDataSourceJobStore
	...
	/**
	 * Register objects in the Scheduler context via a given Map. These objects
	 * will be available to any Job that runs in this Scheduler.
	 * <p>
	 * Note: When using persistent Jobs whose JobDetail will be kept in the
	 * database, do not put Spring-managed beans or an ApplicationContext
	 * reference into the JobDataMap but rather into the SchedulerContext.
	 * 
	 * @param schedulerContextAsMap
	 *            Map with String keys and any objects as values (for example
	 *            Spring-managed beans)
	 * @see JobDetailBean#setJobDataAsMap
	 */
	public void setSchedulerContextAsMap(Map<? extends String, ? extends Object> schedulerContextAsMap) {
	 ...
	 * {@link org.quartz.Job} instances. Note that this default only applies to
	 * a <i>local</i> Scheduler, not to a RemoteScheduler (where setting a
	...
	/**
	 * Set the number of seconds to wait after initialization before starting
	 * the scheduler asynchronously. Default is 0, meaning immediate synchronous
	 * startup on initialization of this bean.
	 * <p>
	 * Setting this to 10 or 20 seconds makes sense if no jobs should be run
	 * before the entire application has started up.
	 */
	...
	/**
	 * Set whether to expose the Spring-managed {@link Scheduler} instance in
	 * the Quartz {@link SchedulerRepository}. Default is "false", since the
	 * Spring-managed Scheduler is usually exclusively intended for access
	 * within the Spring context.
	 * <p>
	 * Switch this flag to "true" in order to expose the Scheduler globally.
	 * This is not recommended unless you have an existing Spring application
	 * that relies on this behavior. Note that such global exposure was the
	 * accidental default in earlier Spring versions; this has been fixed as of
	 * Spring 2.5.6.
	 */
	 ...
	 * jobs at the expense of a longer shutdown phase.
	 * 
	...
	protected Scheduler createScheduler(SchedulerFactory schedulerFactory, String schedulerName)
		...
		boolean overrideClassLoader = (this.resourceLoader != null && !this.resourceLoader.getClassLoader().equals(
	...
	/**
	 * Shut down the Quartz scheduler on bean factory shutdown, stopping all
	 * scheduled jobs.
	 */
